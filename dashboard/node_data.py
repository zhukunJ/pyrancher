...
from k8sapi import k8s  # 使用时间格式化
import re, json
...
def node_resouces(core_api, n_name=None):
    ...
    for node in core_api.list_node().items:
        ...
        node_resouces[node_name]["allocatable_ephemeral_storage"] = round(int(allocatable_storage) / 1024 / 1024 / 1024,
                                                                          2)
        ...
        node_resouces[node_name]["schedulable"] = schedulable
        ...
        for c in pod.spec.containers:
            ...
            if c.resources.requests is not None:
                if "cpu" in c.resources.requests:
                    ...
                    node_resouces[node_name]['cpu_requests'] = round(
                        node_resouces[node_name]['cpu_requests'] + cpuUnitToF(cpu_request), 2)
                if "memory" in c.resources.requests:
                    memory_request = c.resources.requests["memory"]
                    node_resouces[node_name]['memory_requests'] = round(
                        node_resouces[node_name]['memory_requests'] + memoryUnitToG(memory_request), 2)
            # 资源限制
            if c.resources.limits is not None:
                if "cpu" in c.resources.limits:
                    cpu_limit = c.resources.limits["cpu"]
                    node_resouces[node_name]['cpu_limits'] = round(
                        node_resouces[node_name]['cpu_limits'] + cpuUnitToF(cpu_limit), 2)
                if "memory" in c.resources.limits:
                    memory_limit = c.resources.limits["memory"]
                    node_resouces[node_name]['memory_limits'] = round(
                        node_resouces[node_name]['memory_limits'] + memoryUnitToG(memory_limit), 2)
        # 添加Pod数量
...
if __name__ == "__main__":
    ...
    # print(json.dumps(node_resouces(core_api), sort_keys=True, indent=4, separators=(',', ': ')))
    # print(node_resouces(core_api))
    # print(node_info(core_api))

    # for pod in core_api.list_pod_for_all_namespaces().items:
    #     with open('data.txt', 'a') as f:  # 容器中token位置指定在地址在/var/run/secrets/kubernetes.io/serviceaccount
    #          f.write(str(pod))

    for pod in core_api.list_pod_for_all_namespaces().items:
        # print(pod.spec.containers ,"+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
        print(len(pod.spec.containers))
